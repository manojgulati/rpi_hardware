...
static int bcm2835_isp_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_RED_BALANCE:
        printk("here set ctrl red, %0d\n",  ctrl->val );
        if(set_red)
		dev->r_gain = ctrl->val;
		ret = set_wb_gains(node);
		break;
	case V4L2_CID_BLUE_BALANCE:
        printk("here set ctrl blublevel, %0d\n",  ctrl->val );
        if(set_blue)
		dev->b_gain = ctrl->val;
		ret = set_wb_gains(node);
		break;
	case V4L2_CID_DIGITAL_GAIN:
        printk("here: set ctrl digital gain\n"); 
        if(set_dg)
	...
	case V4L2_CID_USER_BCM2835_ISP_CC_MATRIX:
        printk("here: set ctrl came emtrix\n"); 
        if(set_ccmatrix)
	...
	case V4L2_CID_USER_BCM2835_ISP_LENS_SHADING:
	{
		...
		void *vcsm_handle;
        if(set_lenshading){
        printk("here: set ctrl lens shading\n"); 
	...
	case V4L2_CID_USER_BCM2835_ISP_BLACK_LEVEL:
        printk("here set ctrl blevel, %0d\n",  *(ctrl->p_new.p_u8 ));
        if(set_blelvel) 
	...
	case V4L2_CID_USER_BCM2835_ISP_DENOISE:
        *(ctrl->p_new.p_u8) = set_denoise;
        printk("here set ctrl denoise, %0d\n",  *(ctrl->p_new.p_u8 ));
